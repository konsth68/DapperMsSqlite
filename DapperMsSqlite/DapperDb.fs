...
module Util =
    ...
    let convertOptDateTimeToInt (inVal: DateTime option) =
        let res =
            match inVal with
            | Some x -> (x.ToUniversalTime().Ticks - unixStart.Ticks)
            | None -> 0L
...
module DapperDb =
    ...
    type DapperDbObj() =
        ...
        let LogDapperDb str =
            if logFlag then
                DpMsSqlLog.CLog.Information(str)

        member this.SetConnectionString(connectionString: string) = connStr <- connectionString

        member this.SetLogFlag(flag: bool) = logFlag <- flag

        member this.Setup (connectionString: string) (logFlag: bool) =
